...
router.post('/', async (req, res)=>{
    ...
    try {
        ...
        const otp_token = jwt.sign(payload, config.otp_secret);
...
router.post('/verify', async (req, res)=>{
    try {
        let result = await verifyToken(req.body.otp_token, config.otp_secret);

        let current_timestamp = Date.now() / 1000;
        if(current_timestamp > (result.iat + config.otp_validation_time)){
            return res.send(JSON.stringify({
                status: "Failed!",
                action: "OTP expired!"
            }));
